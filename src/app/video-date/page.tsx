...
function VideoDateContent() {
  ...
  const handleSubmitContactDetails = async () => {
    ...
    try {
      ...
      if (shareContactDetails.whatsapp && userContactDetails.whatsapp) {
        ...
        alert(`Failed to encrypt contact details: ${encryptError?.message || 'Unknown error'}. Please try again.`)
        return
      }
      ...
      if (Object.keys(contactDetailsToSave).length > 0) {
        ...
        if (contactError) {
          ...
          alert(`Failed to save contact details: ${errorMessage}. Please check the console for details.`)
          return
        }
      ...
      if (existingExchange) {
        ...
        if (insertError) {
          ...
          alert(`Failed to create contact exchange: ${exchangeErrorInfo.message}. Please try again.`)
          return
        }
  ...
  const enableCameraAndMic = async () => {
    ...
    if (!isRoomConnected()) {
      ...
      alert('Room not connected. Please wait and try again.')
      return
    }
    ...
    if (!engineReady) {
      ...
      if (!engineReady) {
        ...
        alert('Please wait a moment and try again. The connection is still initializing.')
      return
      }
    }
    ...
    if (!navigator.mediaDevices) {
      ...
      alert('Camera/microphone not available. Please use HTTPS or localhost.')
      return
    }
    ...
    try {
      ...
      await room.localParticipant.enableCameraAndMicrophone()
          ...
          if (errorMsg.includes('engine not connected') || 
            ...
            if (attempt < 3) {
              ...
              alert('Connection is still initializing. Please wait a moment and try again.')
              throw enableError
            }
          }
      ...
      if (error.name === 'NotAllowedError') {
        ...
        alert(`Failed to enable camera/microphone: ${error.message || 'Unknown error'}. Please try again.`)
      }
    }
  }
      ...
      if (audioPub?.track?.mediaStreamTrack) {
        ...
        try {
          if (typeof window === 'undefined' || !navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
            alert('Microphone not available. Please use HTTPS or localhost.')
            setIsMuted(!newMutedState)
            return
          }
          ...
          if (err.name === 'NotAllowedError') {
            ...
            alert(`Failed to enable microphone: ${err.message || 'Unknown error'}`)
          }
  ...
  const toggleVideo = async () => {
    ...
    try {
      ...
      if (videoPub?.track?.mediaStreamTrack) {
        ...
        try {
          if (typeof window === 'undefined' || !navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
            alert('Camera not available. Please use HTTPS or localhost.')
            setIsVideoOff(!newVideoOffState)
            return
          }
          ...
          if (err.name === 'NotAllowedError') {
            ...
            alert(`Failed to enable camera: ${err.message || 'Unknown error'}`)
          }
                      ...
                      <motion.button
                        onClick={async () => {
                          ...
                          if (!room || room.state === 'disconnected') {
                            console.log('Room not available for video toggle')
                            return
                          }
                          ...
                          if (videoPub?.track?.mediaStreamTrack) {
                            ...
                            if (!newVideoOffState) {
                              ...
                              if (localVideoRef.current) {
                                console.log('📹 Countdown: Attaching existing track to video element')
                            ...
                            try {
                              ...
                              // Set track immediately so video shows in countdown
                              setLocalVideoTrack(videoTrack)
                              setCameraMicEnabled(true)
                              console.log('✅ Camera enabled and published')
                            } catch (err: any) {
