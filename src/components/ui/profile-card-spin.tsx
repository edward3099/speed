...
export function ProfileCardSpin({
  ...
  return (
    <motion.div
      className={cn(
        "w-full max-w-[140px] sm:max-w-xs md:max-w-sm p-1.5 sm:p-5 md:p-6 rounded-md sm:rounded-xl md:rounded-2xl bg-white/5 backdrop-blur-sm",
    ...
    >
      ...
      <motion.h3
        className="text-xs sm:text-xl md:text-2xl font-bold text-white mb-0.5 sm:mb-1 md:mb-2"
        animate={isSelected ? {
          textShadow: [
      ...
      <p className="text-[9px] sm:text-sm opacity-80 leading-tight sm:leading-relaxed line-clamp-1 sm:line-clamp-none">{bio}</p>
    </motion.div>
  )
}
