...
BEGIN
  ...
  ELSE
    ...
    IF v_both_voted THEN
      ...
      ELSIF v_user1_vote = 'pass' AND v_user2_vote = 'pass' THEN
        v_outcome := 'pass_pass';
      ELSIF (v_user1_vote = 'pass' AND v_user2_vote IS NULL) OR (v_user1_vote IS NULL AND v_user2_vote = 'pass') THEN
        -- One user voted pass, other hasn't voted yet - treat as pass_pass (both respin)
        v_outcome := 'pass_pass';
      ELSE
        -- Fallback
        v_outcome := 'pass_pass';
  ...
  IF v_resolved THEN
    ...
    ELSIF v_outcome = 'yes_pass' THEN
      -- If one user votes pass, BOTH users go to respin (re-queue)
      -- Yes user gets +10 fairness boost
      ...
      -- Auto-requeue BOTH users (one voted pass, so both respin)
      PERFORM join_queue(v_yes_user_id);
      PERFORM join_queue(v_pass_user_id);
      
    ELSE
      -- pass_pass or idle_idle - both go to idle, then auto-requeue
      UPDATE users_state
      SET
        state = 'idle',
        partner_id = NULL,
      ...
      -- Auto-requeue both users (they voted pass, so they want to respin)
      PERFORM join_queue(v_match.user1_id);
      PERFORM join_queue(v_match.user2_id);
    END IF;
