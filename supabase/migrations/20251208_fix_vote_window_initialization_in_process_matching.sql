-- ============================================================================
-- Fix Vote Window Initialization in process_matching
-- ============================================================================
-- CRITICAL FIX: Initialize vote_window_expires_at immediately after match creation
-- This prevents matches from getting stuck in 'paired' state without a vote window
-- Based on analysis finding 870 stuck matches affecting 1,740 users
-- ============================================================================
-- 
-- Problem: process_matching() creates matches but doesn't initialize vote_window_expires_at
-- Result: Matches get stuck in 'paired' state, users can't vote or progress
-- 
-- Solution: Initialize vote window immediately after match creation (like create_match_atomic does)
-- ============================================================================
...
BEGIN
  ...
  -- Activity priority is only a tiebreaker, not primary sort
  FOR v_user1 IN
    SELECT 
      user_id,
      fairness,
...
COMMENT ON FUNCTION process_matching IS 'Fixed: Initializes vote_window_expires_at immediately after match creation to prevent stuck matches. Processes up to 10 matches per cycle. Batch size: 150 users.';
