-- Fix race condition in record_vote when both users vote simultaneously
-- Issue: When both users vote at the same time, the re-read might not see the other vote
-- Solution: Add retry mechanism with small delay to ensure we see both votes
