-- Add comprehensive logging to matching functions (fixed)
...
BEGIN
  ...
  SELECT u.gender, COALESCE(up.gender_preference, 'both') INTO user_gender, user_gender_pref
  FROM profiles u
  ...
  VALUES ('find_best_match_start', jsonb_build_object(
    ...
    'gender_preference', user_gender_pref,
  ...
  WHERE q.user_id != p_user_id
    ...
    AND (up.gender_preference = user_gender OR up.gender_preference = 'both' OR up.gender_preference IS NULL)  -- Candidate wants user's gender
    AND (user_gender_pref = u.gender OR user_gender_pref = 'both');  -- User wants candidate's gender
  ...
  WHERE q.user_id != p_user_id
    ...
    AND (up.gender_preference = user_gender OR up.gender_preference = 'both' OR up.gender_preference IS NULL)
    AND (user_gender_pref = u.gender OR user_gender_pref = 'both')
