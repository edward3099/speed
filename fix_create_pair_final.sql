-- Force fix create_pair_atomic - return UUID to match matches.id column type
DROP FUNCTION IF EXISTS create_pair_atomic(UUID, UUID);

CREATE FUNCTION create_pair_atomic(
  p_user1_id UUID,
  p_user2_id UUID
) RETURNS UUID
LANGUAGE plpgsql
